...
func (d Factory) CreateProvider(ctx kore.ServiceProviderContext, provider *servicesv1.ServiceProvider) (_ kore.ServiceProvider, complete bool, _ error) {
	...
	defaultAppPlan := servicesv1.ServicePlan{
		...
		Spec: servicesv1.ServicePlanSpec{
			...
			Description: "Default plan for Kubernetes applications",
		},
	}
	if err := defaultAppPlan.Spec.SetConfiguration(AppConfiguration{
		Resources: Resources{},
		Values:    YAMLMap{},
	}); err != nil {
		panic(fmt.Errorf("failed to create default app plan: %w", err))
	}

	plans = append(plans, defaultAppPlan)

	defaultHelmAppPlan := servicesv1.ServicePlan{
		TypeMeta: metav1.TypeMeta{
			Kind:       "ServicePlan",
			APIVersion: servicesv1.GroupVersion.String(),
		},
		...
		Spec: servicesv1.ServicePlanSpec{
			Kind:        ServiceKindHelmApp,
			Labels:      nil,
			Summary:     "Kubernetes Helm application",
			Description: "Default plan for Kubernetes Helm applications",
		},
	}
	if err := defaultHelmAppPlan.Spec.SetConfiguration(HelmAppConfiguration{
		Source: HelmAppSource{},
		Values: YAMLMap{},
	}); err != nil {
		panic(fmt.Errorf("failed to create default app plan: %w", err))
	}

	plans = append(plans, defaultAppPlan, defaultHelmAppPlan)
